...
def compare_with_orig(serializer, song):
  ranges=[]
  ok = True
  spc = util.open_asset('sound/%s.spc' % song, 'rb').read()
...
def print_song(song):
  ...
  sorted_ents = process_file(util.open_asset('sound_%s.txt' % song, 'r'))
  if song == 'intro':
    sorted_ents.extend(process_file(util.open_asset('sfx.txt', 'r')))
