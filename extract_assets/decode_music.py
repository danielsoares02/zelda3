...
def dump_brr_audio():
  ...
  for audio_idx in range(25):
    ...
    util.open_asset('sound/sound%d.pcm.brr' % audio_idx, 'wb').write(bytes(brr_data))
    util.open_asset('sound/sound%d.pcm' % audio_idx, 'wb').write(sound_data)
...
def dump_music_info():
  ...
  util.open_asset('music_info.yaml', 'w').write(s)
...
def extract_sound_data(rom):
  load_song(rom, 'intro')
  print_song('intro', util.open_asset('sound_intro.txt', 'w'))

  dump_brr_audio()
  dump_music_info()
  print_all_sfx(util.open_asset('sfx.txt', 'w'))

  load_song(rom, 'indoor')
  print_song('indoor', util.open_asset('sound_indoor.txt', 'w'))

  load_song(rom, 'ending')
  print_song('ending', util.open_asset('sound_ending.txt', 'w'))
