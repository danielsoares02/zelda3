...
def print_overworld_area(overworld_area):
  ...
  util.open_asset('overworld/overworld-%d.yaml' % overworld_area, 'w').write(s)
...
def print_dialogue():
  text_compression.print_strings(util.ROM, file = util.open_asset(text_compression.dialogue_filename(util.ROM.language), 'w', encoding='utf8'))
...
def print_all_dungeon_rooms():
  for i in range(320):
    s = print_room(i)
    util.open_asset('dungeon/dungeon-%d.yaml' % i, 'w').write(s)
...
def print_default_rooms():
  ...
  util.open_asset('dungeon/default_rooms.yaml', 'w').write(s)
...
def print_overlay_rooms():
  ...
  s = yaml.dump(default_rooms, default_flow_style=None, sort_keys=False)
  util.open_asset('dungeon/overlay_rooms.yaml', 'w').write(s)

def make_directories():
  os.makedirs(util.get_assets_file_path('img'), exist_ok=True)
  os.makedirs(util.get_assets_file_path('overworld'), exist_ok=True)
  os.makedirs(util.get_assets_file_path('dungeon'), exist_ok=True)
  os.makedirs(util.get_assets_file_path('sprites'), exist_ok=True)
  os.makedirs(util.get_assets_file_path('sound'), exist_ok=True)
...
def print_all_text_stuff():
  ...
  print_default_rooms()
  print_dialogue()
  print_map32_to_map16(util.open_asset('map32_to_map16.txt', 'w'))
