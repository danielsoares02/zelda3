...
def save_as_png(dimensions, data, fname, palette = None):
  ...
  img.save(util.get_assets_file_path(fname))
...
def decode_hud_icons():
  class PaletteUsage:
    def __init__(self):
      self.data = open('../other/palette_usage.bin', 'rb').read()
    def get(self, icon):
...
def encode_font_from_png(lang):
  font_data = Image.open(util.get_assets_file_path(kFontTypes[lang][2])).tobytes()
...
def load_sprite_sheets():
  master_tilesheets = MasterTilesheets()
  for char in "0123456789ABCDEFX":
    img = Image.open(util.get_assets_file_path('sprites/sprites_%s.png' % char))
    img_size = img.size
