...
def print_map32_to_map16():
  tab = {}
  for line in util.open_asset('map32_to_map16.txt'):
    line = line.strip()
    x, xs = line.split(':', 1)
...
def compress_dialogue(fname, lang):
  lines = []
  for line in util.open_asset(fname, encoding='utf8').read().splitlines():
    a, b = line.split(': ', 1)
    lines.append(b)
...
def print_dialogue(args):
  ...
  if args.languages:
    for a in args.languages.split(','):
      ...
      if not os.path.exists(util.get_assets_file_path(name)):
        raise Exception(f'{util.get_assets_file_path(name)} not found. You need to extract it with --extract-dialogue using the ROM of that language.')
      languages.append(a)
...
def load_overworld_yaml(room):
  return yaml.safe_load(util.open_asset('overworld/overworld-%d.yaml' % room, 'r'))
...
def load_dungeon_yaml(room):
  return yaml.safe_load(util.open_asset('dungeon/dungeon-%d.yaml' % room, 'r'))
...
def print_dungeon_rooms():
  ...
  data = []
  offsets = [0] * 8
  default_yaml = yaml.safe_load(util.open_asset('dungeon/default_rooms.yaml', 'r'))
  ...
  data = []
  offsets = [0] * 19
  overlay_yaml = yaml.safe_load(util.open_asset('dungeon/overlay_rooms.yaml', 'r'))
  for i in range(len(offsets)):
    offsets[i] = len(data)
...
def print_link_graphics():
  image = Image.open(util.get_assets_file_path('linksprite.png'))
  data = image.tobytes()
...
def write_assets_to_file(print_header = False, dat_path = None):
  key_sig = b''
  all_data = []
  if print_header:
    print('''#pragma once
...
''' % len(assets))
  ...
  open(os.path.join(dat_path, 'zelda3_assets.dat'), 'wb').write(file_data)
